...
## Ranking autoalojado (API REST + SQLite)

El ranking usa `localStorage` por defecto para que cada dispositivo mantenga sus
puntuaciones. El ranking compartido se sirve desde una API REST propia
(`edumusic.edutictac.es/api/`), que sustituye a Firebase Firestore
(proyecto `edumusic-f67e0`, retirado 2026-09).

- El cliente (`js/score-service.js`) habla con la API vía `fetch` y mantiene un
  fallback local en caso de perder conexión.
- La URL de la API se puede sobreescribir con `window.EDUMUSIC_API_BASE` o
  `ScoreService.setApiBase('...')` (por defecto `/api`, mismo origen).
- El backend es un microservicio FastAPI + SQLite. Código en el repo
  `Edutictac/edumusic-api` (GitHub y Forgejo).
